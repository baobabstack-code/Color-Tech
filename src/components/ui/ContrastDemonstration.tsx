            ...
            <div className="px-4 py-3 bg-white dark:bg-gray-900 flex flex-wrap gap-4 items-center">
              {showContrastRatio && (
                <div>
                  ...
                  <span className={`text-sm ${getContrastColor(contrastRatio)}`}>
              ...
              {showWCAGLevel && (
                <div>
                  ...
                  <span className={`text-sm ${getWCAGLevelColor(wcagLevel)}`}>
                    {wcagLevel}
                  </span>
                </div>
              )}
              ...
              {showPassFail && (
                <div>
                  ...
                  <span className={`text-sm ${passes ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    {passes ? 'Pass ✓' : 'Fail ✗'}
                  </span>
                </div>
              )}
...
export function TextSpacingDemonstration({
  ...
  const spacingSamples = [
    { lineHeight: 'leading-none', letterSpacing: 'tracking-normal', label: 'No Line Height (1)' },
    { lineHeight: 'leading-tight', letterSpacing: 'tracking-normal', label: 'Tight Line Height (1.25)' },
    { lineHeight: 'leading-normal', letterSpacing: 'tracking-normal', label: 'Normal Line Height (1.5)' },
    { lineHeight: 'leading-relaxed', letterSpacing: 'tracking-normal', label: 'Relaxed Line Height (1.75)' },
    { lineHeight: 'leading-loose', letterSpacing: 'tracking-normal', label: 'Loose Line Height (2)' },
    { lineHeight: 'leading-normal', letterSpacing: 'tracking-tighter', label: 'Tighter Letter Spacing (-0.05em)' },
    { lineHeight: 'leading-normal', letterSpacing: 'tracking-tight', label: 'Tight Letter Spacing (-0.025em)' },
    { lineHeight: 'leading-normal', letterSpacing: 'tracking-normal', label: 'Normal Letter Spacing (0)' },
    { lineHeight: 'leading-normal', letterSpacing: 'tracking-wide', label: 'Wide Letter Spacing (0.025em)' },
    { lineHeight: 'leading-normal', letterSpacing: 'tracking-wider', label: 'Wider Letter Spacing (0.05em)' },
  ];
    ...
    <div className={`border rounded-lg overflow-hidden shadow-sm ${className}`}>
      ...
      >
        {spacingSamples.map((sample, index) => (
          <div key={index} className="space-y-1">
            <p className={`text-base ${sample.lineHeight} ${sample.letterSpacing}`}>
              The quick brown fox jumps over the lazy dog. The five boxing wizards jump quickly.
              This paragraph demonstrates the effect of different line heights and letter spacing on text readability.
            </p>
            <p className="text-xs opacity-75">{sample.label}</p>
          </div>
        ))}
...
function getWCAGLevelColor(level: string): string {
  switch (level) {
    ...
    case 'AA': return 'text-green-600 dark:text-green-400';

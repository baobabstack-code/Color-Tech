  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse" aria-label="Color contrast grid showing ratios between colors">
          <caption className="sr-only">
            Color contrast grid showing contrast ratios between foreground (rows) and background (columns) colors.
            {textSize === 'normal' ? 'Normal text requires 4.5:1 contrast ratio.' : 'Large text requires 3:1 contrast ratio.'}
          </caption>
          
          <thead>
            <tr>
              <th scope="col" className="p-2 border text-left font-medium">Foreground / Background</th>
              {colors.map((color) => (
                <th 
                  key={color.value} 
                  scope="col" 
                  className="p-2 border text-center font-medium"
                  style={{ backgroundColor: color.value, color: getContrastRatio(color.value, '#ffffff') > getContrastRatio(color.value, '#000000') ? '#ffffff' : '#000000' }}
                >
                  <div>{color.name}</div>
            ...
            {colors.map((rowColor) => (
              <tr key={rowColor.value}>
                <th 
                  ...
                  style={{ backgroundColor: rowColor.value, color: getContrastRatio(rowColor.value, '#ffffff') > getContrastRatio(rowColor.value, '#000000') ? '#ffffff' : '#000000' }}
                >
                ...
                {colors.map((colColor) => {
                  const contrastRatio = getContrastRatio(rowColor.value, colColor.value);
                  const wcagLevel = meetsContrastStandard(contrastRatio, textSize === 'large');
                  const isSelected = selectedColor === `${rowColor.value}-${colColor.value}`;
                  ...
                  return (
                    <td 
                      ...
                      onClick={() => setSelectedColor(isSelected ? null : `${rowColor.value}-${colColor.value}`)}
                      aria-selected={isSelected}
                    >
                      <div className={`font-medium ${getCellTextColor(contrastRatio)}`}>
      ...
      {selectedColor && (
        <div className="mt-6">
          <h4 className="text-md font-medium mb-2">Selected Color Combination</h4>
 ...
 */
function ColorCombinationPreview({ foreground, background, textSize }: ColorCombinationPreviewProps) {
    ...
    <div className="border rounded-lg overflow-hidden">
      ...
      >
        ...
        <p className="mb-4">
          ...
          The contrast ratio is {contrastRatio.toFixed(2)}:1, which {wcagLevel !== 'Poor' ? 'meets' : 'does not meet'} WCAG {wcagLevel} standards.
        </p>
      ...
      <div className="p-4 bg-white dark:bg-gray-800">
        ...
        <div className="mt-4">
          ...
          <div className="flex items-center mt-1">
            ...
            <span 
              className={`${wcagLevel === 'AAA' ? 'text-green-600 dark:text-green-400' : wcagLevel === 'AA' || wcagLevel === 'AA Large' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
              {wcagLevel}
            </span>
          </div>

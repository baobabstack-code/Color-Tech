...
const CustomerForm = ({
  ...
  onSave: (
    customer: Omit<
      Customer,
      "id" | "bookingCount" | "lastActivity" | "createdAt"
    >
  ) => void;
  onCancel: () => void;
}) => {
...
export default function CustomerManagement() {
  ...
  const handleSaveCustomer = async (
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `Failed to ${selectedCustomer ? "update" : "create"} customer.`
        );

      toast({
        title: "Success",
        description: `Customer successfully ${selectedCustomer ? "updated" : "created"}.`,
      });
      closeModal();
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCustomers.map((customer) => (
            <Card
              key={customer.id}
              className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50 p-6 hover:bg-slate-800/70 transition-all duration-200 flex flex-col justify-between"
            >
              <div>

...
async function seedDatabase() {
    ...
    `, ['staff@colortech.com', hashedPassword, 'Staff', 'User', 'staff', '1234567891', true]);
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const phone = faker_1.faker.string.numeric(10); // Generate a 10-digit phone number
            await client.query(`
        INSERT INTO users (email, password, first_name, last_name, role, phone, is_active, created_at, updated_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
        ON CONFLICT (email) DO NOTHING
      ...
      `, [category.name, category.description]);
        ...
        for (const clientId of clientIds) {
            ...
            for (let i = 0; i < numVehicles; i++) {
                ...
                const model = faker_1.faker.vehicle.model();
                const year = faker_1.faker.number.int({ min: 2010, max: 2023 });
                const color = carColors[Math.floor(Math.random() * carColors.length)];
                const licensePlate = faker_1.faker.vehicle.vrm();
                const vin = faker_1.faker.vehicle.vin();
                await client.query(`
